...
def process_mov_in_thread(pid_file_path, hmb_file_path, pid_filename, user_email, job_id):
    ...
    """
    log_and_print(f"🚀 [MOV Thread {job_id[:8]}] Starting processing...")
    ...
    try:
        ...
        log_and_print(f"👁️ [MOV {job_id[:8]}] STEP 2: Extracting HMB with Vision...")
        try:
        ...
        cache.set(f'mov_task_{job_id}_progress', 60, timeout=3600)
        cache.set(f'mov_task_{job_id}_stage', 'Matching P&ID and HMB data...', timeout=3600)
        
        # STEP 3: Match lines
        log_and_print(f"🔗 [MOV {job_id[:8]}] STEP 3: Matching lines...")
        line_context = match_lines_to_streams(pid_data, hmb_data)
        log_and_print(f"✅ [MOV {job_id[:8]}] Lines matched")
        ...
        log_and_print(f"📈 [MOV {job_id[:8]}] STEP 5: Generating Excel...")
        generator = MOVExcelGeneratorDynamic()
        excel_buffer = generator.generate_datasheet(mapped_data)
        excel_bytes = excel_buffer.getvalue()
        excel_base64 = base64.b64encode(excel_bytes).decode('utf-8')
        log_and_print(f"✅ [MOV {job_id[:8]}] Excel generated: {len(excel_bytes)} bytes")
