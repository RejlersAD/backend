...
def live_activity(organization_id=None, search='', state='', page=1, page_size=10, selected_user=None, now=None,
                  module='', ordering='signal', timeline_limit=20):
    ...
    modules = sorted(set(events.values_list('application', flat=True)) | set(calls.values_list('application', flat=True)) |
                     set(workflows.filter(Q(started_at__gte=start) | Q(finished_at__gte=start) | Q(status='running')).values_list('module', flat=True)))
    if module:
        events, calls, workflows = events.filter(application=module), calls.filter(application=module), workflows.filter(module=module)
    if search:
    ...
    people = people.annotate(
        ...
        running=Exists(running), stalled=Exists(stale), workflow_checks=count(stale), running_module=Subquery(running.values('module')[:1]),
    ...
    annotated_people = people
    if module:
        people = people.filter(Q(last_signal__isnull=False) | Q(running=True) | Q(stalled=True))
    # Summary reflects search/module, before the state filter; states are mutually exclusive.
    ...
    sort = {'user': ['first_name', 'last_name', 'pk'], '-user': ['-first_name', '-last_name', 'pk'],
            'state': ['priority', 'first_name', 'pk'], '-state': ['-priority', 'first_name', 'pk'],
            'latest': [F('last_signal').desc(nulls_last=True), 'pk'], '-latest': [F('last_signal').asc(nulls_last=True), 'pk']}
    rows = rows.order_by(*sort.get(ordering, ['priority', F('last_signal').desc(nulls_last=True), 'last_name', 'first_name', 'pk']))
    rows = rows.select_related('rbac_profile')[(page - 1) * page_size:page * page_size]
    ...
    def serialize(user):
        ...
        return {'id': str(user.pk), 'profile_id': str(user.rbac_profile.pk), 'name': user.get_full_name() or user.email, 'email': user.email,
                        ...
                        'ai_calls_24h': user.calls_24h, 'failed_ai_calls_24h': user.failed_calls_24h, 'workflow_checks': user.workflow_checks}
    results = [serialize(user) for user in rows]
    detail = None
    if selected_user:
        user = annotated_people.filter(pk=selected_user).select_related('rbac_profile').first()
        if user is None:
            raise NotFound('User is not available in this reporting scope.')
        timeline = []
        for r in events.filter(user=user).order_by('-timestamp', '-pk')[:timeline_limit]:
        ...
        for r in calls.filter(user=user).order_by('-timestamp', '-pk')[:timeline_limit]:
        ...
        for r in workflows.filter(user=user).filter(Q(started_at__gte=start) | Q(finished_at__gte=start) | Q(status='running')).annotate(observed=Coalesce('finished_at', 'started_at')).order_by('-observed', '-pk')[:timeline_limit]:
        ...
        detail = {**serialize(user), 'timeline': timeline[:timeline_limit], 'limit': timeline_limit,
                  'activity_log': [r for r in timeline if r['source'] == 'Submitted activity'],
                  'ai_requests': [r for r in timeline if r['source'] == 'Server AI request']}
    return {'generated_at': now, 'window_start': start, 'refresh_seconds': 15, 'recent_minutes': 5,
            ...
            'summary': summary, 'modules': [m for m in modules if m], 'count': total, 'page': page, 'page_size': page_size, 'results': results, 'selected': detail}
