...
def complete_catalogue(apps, schema_editor):
    ensure_module_actions(apps.get_model('rbac', 'Module'), apps.get_model('rbac', 'Permission'),
                          using=schema_editor.connection.alias)


class Migration(migrations.Migration):
    dependencies = [('rbac', '0053_user_permission_overrides')]
    # Definitions may acquire grants/overrides later, so rollback must retain them.
    operations = [migrations.RunPython(complete_catalogue, migrations.RunPython.noop)]
