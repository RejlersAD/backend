...
# Soft-coded: PIL's default MAX_IMAGE_PIXELS (~178 megapixels) is a heuristic
# decompression-bomb guard for arbitrary untrusted image uploads. Here the
# images are rendered in-memory from a page of a PDF we already validated and
# stored (not an untrusted external image blob), so the DoS risk doesn't
# apply — but the default limit still blocks legitimate large-format
# engineering sheets (e.g. ARCH D/E size) at the 450 DPI OCR pass. Raise it
# generously rather than disabling the check outright.
_PIL_MAX_IMAGE_PIXELS = 400_000_000

# Soft-coded: adaptive OCR early-exit thresholds (used by _run_ocr).
# Without these, every page runs all 3 DPIs x 3 image variants x 4 Tesseract
# configs = 36 full-page OCR calls unconditionally (up to 450 DPI), even for
# clean/clear scans that a single cheap 150-DPI pass already reads fine.
# Escalation only continues past a tier when the accumulated yield is still
# below these thresholds, so genuinely faint/complex pages still get the
# full 36-pass treatment (behavior unchanged for the hard case) while clean
# pages exit early (dramatically fewer passes for the common case).
_OCR_MIN_LINES_FOR_EARLY_EXIT = 15
_OCR_MIN_TAGS_FOR_EARLY_EXIT = 5


def _ocr_yield_sufficient(seen_lines: set) -> bool:
    """Heuristic: is the OCR text accumulated so far 'good enough' to stop escalating?"""
    if len(seen_lines) < _OCR_MIN_LINES_FOR_EARLY_EXIT:
        return False
    combined = '\n'.join(seen_lines)
...
def _run_tesseract_ocr(file_path: str, page_index: int) -> str:
    ...
    try:
        ...
        # Dict keyed by DPI so higher DPIs are only rendered/decoded if the
        # adaptive escalation below actually needs them (rendering a 450 DPI
        # page is itself expensive — no point paying for it on pages that
        # resolve fine at 150 DPI).
        dpi_levels = (150, 300, 450)
        page_by_dpi = {}
        if ext == 'pdf':
            ...
            page_by_dpi = {dpi: None for dpi in dpi_levels}  # lazy: rendered on first use
            _doc_ref = doc
        else:
            _doc_ref = None
            page_by_dpi = {dpi_levels[0]: Image.open(file_path).convert('L')}
        ...
        def _scan() -> str:
            ...
            # threshold. Clean/clear scans (the common case) typically exit
            # after 1-4 passes instead of unconditionally running all 36
            # (3 DPIs x 3 variants x 4 configs) full-page Tesseract calls;
            # genuinely faint/complex pages still escalate all the way up,
            # preserving the original worst-case extraction quality.
            for dpi in dpi_levels:
                if dpi not in page_by_dpi:
                    continue
                if page_by_dpi[dpi] is None:
                    page_by_dpi[dpi] = _render(dpi)
                for variant in _variants(page_by_dpi[dpi]):
                    for cfg in TESSERACT_CONFIGS:
                        _absorb(pytesseract.image_to_string(variant, config=cfg))
                        if _ocr_yield_sufficient(seen_lines):
                            return '\n'.join(all_text_parts)
        ...
        # continuing (with empty text) until the hard time limit SIGKILLs
        # the worker process.
        raise
    except Exception as exc:
