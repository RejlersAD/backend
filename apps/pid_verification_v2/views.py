...
def export_excel(request, document_id):
    ...
    from .services.export_service import generate_excel, COMPARISON_VIEWS
    view = request.query_params.get("view", "all")
    if view not in COMPARISON_VIEWS:
        return Response({"error": f"Invalid view '{view}'"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    data = generate_excel(doc, view=view)
    if not data:
    ...
    response["Content-Disposition"] = f'attachment; filename="pidv_findings_{safe_name}{suffix}.xlsx"'
    return response
...
def export_pdf(request, document_id):
    ...
    from .services.export_service import generate_pdf, COMPARISON_VIEWS
    view = request.query_params.get("view", "all")
    if view not in COMPARISON_VIEWS:
        return Response({"error": f"Invalid view '{view}'"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    # Always regenerate in-memory so the response is served from the same origin
    # (avoids S3 CORS errors that occur when axios follows a cross-origin redirect).
    data = generate_pdf(doc, view=view)
    if not data:
    ...
    response["Content-Disposition"] = f'attachment; filename="pidv_report_{safe_name}{suffix}.pdf"'
    return response
    ...
    if project_id:
        ...
        if project_id_str:
            try:
                ...
                qs = qs.filter(project__id=project_id_str)
                logger.info(
...
def update_document(request, document_id):
    """
    PATCH /api/v2/pid-verification/update/<document_id>/

    Soft-coded: DOCUMENT_EDITABLE_FIELDS lists the only PIDVDocument fields a
    user may edit from the "Previous Uploads" list. Currently just the
    display name — extend this set (and the per-field validation below) if
    more editable fields are needed later.
    """
    doc = _get_doc_or_404(document_id, request.user)
    if doc is None:
        return Response({"error": "Document not found"}, status=status.HTTP_404_NOT_FOUND)

    DOCUMENT_EDITABLE_FIELDS = {'file_name'}
    updates = {k: v for k, v in request.data.items() if k in DOCUMENT_EDITABLE_FIELDS}
    if not updates:
        return Response(
            {"error": f"No editable fields provided. Editable fields: {sorted(DOCUMENT_EDITABLE_FIELDS)}"},
            status=status.HTTP_400_BAD_REQUEST,
        )

    if 'file_name' in updates:
        new_name = str(updates['file_name']).strip()
        if not new_name:
            return Response({"error": "file_name cannot be empty"}, status=status.HTTP_400_BAD_REQUEST)
        if len(new_name) > 512:
            return Response({"error": "file_name must be 512 characters or fewer"}, status=status.HTTP_400_BAD_REQUEST)
        doc.file_name = new_name

    doc.save(update_fields=list(updates.keys()) + ['updated_at'])

    logger.info("[PIDVUpdate] document_id=%s updated fields=%s by user=%s", document_id, list(updates.keys()), request.user)
...
def project_legend(request, project_id):
    ...
    """
    try:
        # FIXED: Use project_id (UUID) field instead of id
...
def project_legend_sheets(request, project_id):
    ...
    for s in sheets:
        ...
        from apps.pid_verification_v2.models import PIDVInstrumentSymbol
...
def upload_reference_data(request, project_id):
    ...
    # Queue parsing task for Excel/CSV/PDF files (task supports all three via
    # pandas for Excel/CSV, pdfplumber table extraction for PDF)
    if ext in {'.xlsx', '.xls', '.csv', '.pdf'}:
        ...
        # .txt files — no parser implemented yet, mark as completed (no parsing)
...
@api_view(["GET", "POST"])
@permission_classes([IsAuthenticated, HasDisciplineAccess])
def manage_api_keys(request, project_id):
    """
    GET  /api/v2/pid-verification/projects/<project_id>/api-keys/
    POST /api/v2/pid-verification/projects/<project_id>/api-keys/
    
    Manage encrypted API keys for AI deep extraction (OpenAI, Claude).
    Keys are stored encrypted per project for security.
    
    GET returns key status (active/not_set) without exposing actual keys.
    POST saves encrypted keys.
    ...
    """
    manage_api_keys.module_required = 'pid_verification'
    ...
    if request.method == 'GET':
        ...
        return Response({
            'api_keys': {
                'openai_key': bool(api_keys.get('openai_key')),
                'claude_key': bool(api_keys.get('claude_key')),
            }
        }, status=status.HTTP_200_OK)
    ...
    if openai_key:
        ...
        metadata['api_keys']['openai_key'] = encrypted_openai
    
    if claude_key:
        encrypted_claude = fernet.encrypt(claude_key.encode()).decode()
        metadata['api_keys']['claude_key'] = encrypted_claude
    ...
    return Response({
        ...
        'api_keys': {
            'openai_key': bool(metadata['api_keys'].get('openai_key')),
            'claude_key': bool(metadata['api_keys'].get('claude_key')),
        }
    }, status=status.HTTP_200_OK)
...
@api_view(["DELETE"])
@permission_classes([IsAuthenticated, HasDisciplineAccess])
def delete_api_key(request, project_id, provider):
    """
    DELETE /api/v2/pid-verification/projects/<project_id>/api-keys/<provider>/
    
    Remove a specific API key (provider: 'openai' or 'claude').
    """
    delete_api_key.module_required = 'pid_verification'
    ...
    if provider not in ['openai', 'claude']:
        return Response({'error': 'Invalid provider. Must be "openai" or "claude"'}, status=status.HTTP_400_BAD_REQUEST)
    
    metadata = project.metadata or {}
    api_keys = metadata.get('api_keys', {})
    
    key_field = f'{provider}_key'
    if key_field in api_keys:
        del api_keys[key_field]
        metadata['api_keys'] = api_keys
        project.metadata = metadata
        project.save(update_fields=['metadata'])
    
    return Response({'message': f'{provider.capitalize()} API key removed'}, status=status.HTTP_200_OK)
...
def run_ai_checks(request, project_id):
    ...
    """
    from .models import PIDVProject, PIDVAICheckRun, PIDVDocument
    from .tasks import run_ai_checks_task
...
def project_ai_check_runs(request, project_id):
    ...
    """
    from .models import PIDVProject, PIDVAICheckRun
...
def ai_check_run_detail(request, run_id):
    ...
    """
    from apps.pid_verification_v2.models import PIDVAICheckRun
...
def drawing_image(request, document_id, page_index):
    ...
    Soft-coded / storage-agnostic: reads the file through Django's storage
    API (`.open()` / `.read()`) instead of `.path`, so this works whether
    `original_file` lives on local disk (dev) or on S3 (production / any
    environment with USE_S3=True). `.path` raises NotImplementedError on
    non-filesystem storages, which used to be silently swallowed and shown
    to the user as a generic "Drawing preview unavailable" message.
    """
    doc = _get_doc_or_404(document_id, request.user)
    if doc is None:
        return Response({"error": "Document not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    if not doc.original_file:
        logger.warning("[PIDVDrawingImage] document_id=%s has no original_file stored", document_id)
        return Response({"error": "Original file not stored"}, status=status.HTTP_404_NOT_FOUND)

    try:
        with doc.original_file.open("rb") as f:
            raw = f.read()
    except FileNotFoundError:
        logger.error(
            "[PIDVDrawingImage] Source file missing on storage for document_id=%s name=%s",
            document_id, doc.original_file.name,
        )
        return Response(
        ...
        logger.error(
            "[PIDVDrawingImage] Failed to read original_file for document_id=%s: %s",
            document_id, exc, exc_info=True,
        )
        return Response({"error": f"Could not read source file: {exc}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    ext = Path(doc.original_file.name).suffix.lower().lstrip(".")
    png_data = None
    ...
    if ext == "pdf":
        try:
            ...
            logger.error("[PIDVDrawingImage] PDF render failed for document_id=%s: %s", document_id, exc, exc_info=True)
            return Response({"error": f"Failed to render PDF page: {exc}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            logger.error("[PIDVDrawingImage] Image read failed for document_id=%s: %s", document_id, exc, exc_info=True)
            return Response({"error": f"Failed to read image file: {exc}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    else:
...
def project_instrument_symbols(request, project_id):
    ...
    """
    # NOTE: use the V2 app's own PIDVInstrumentSymbol/registry — V2's PIDVProject
    # is a distinct model from V1's, so V1's FK-typed queryset would raise
    # "Cannot query ...: Must be 'PIDVProject' instance." for a V2 project.
    from apps.pid_verification_v2.models import PIDVInstrumentSymbol
    from apps.pid_verification_v2.services.instrument_registry import get_category_counts
...
def instrument_symbol_detail(request, symbol_id):
    ...
    """
    # NOTE: use the V2 app's own PIDVInstrumentSymbol — see project_instrument_symbols() above.
    from apps.pid_verification_v2.models import PIDVInstrumentSymbol
...
def _get_project_or_404(project_id: str, user):
    ...
    """
    try:
        # FIXED: Use project_id (UUID) field instead of id (auto-increment PK)
        project = PIDVProject.objects.get(project_id=project_id)
        user_obj = getattr(user, "user", user)
