"""Concurrency-safe Goods Receipt number allocation."""
...
GR_NUMBER_PATTERN = re.compile(r'^RAD-GR-(\d+)_(\d{4})$')
...
class GoodsReceiptNumberService:
    ...
    def _largest_existing_value(cls, year):
        largest = 0
        for number in Receipt.objects.filter(
            receipt_number__startswith='RAD-GR-',
            receipt_number__endswith=f'_{year}',
        ).values_list('receipt_number', flat=True):
            match = GR_NUMBER_PATTERN.fullmatch(str(number or ''))
    ...
    def next_number(cls, year=None):
        ...
        sequence.last_value = max(sequence.last_value, cls._largest_existing_value(year)) + 1
