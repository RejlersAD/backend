...
def build_activities(project, wbs: list, intelligence: dict) -> dict:
    ...
    m_award = _milestone_activity(ids, MILESTONE_TEMPLATE[0], effective_date)
    m_kickoff_date = add_working_days(effective_date, 1, calendar['working_days_per_week'])
    m_kickoff = _milestone_activity(ids, MILESTONE_TEMPLATE[1], m_kickoff_date, m_award['id'])
    m_mobilize_date = add_working_days(m_kickoff_date, 3, calendar['working_days_per_week'])
    m_mobilize = _milestone_activity(ids, MILESTONE_TEMPLATE[2], m_mobilize_date, m_kickoff['id'])
    activities += [m_award, m_kickoff, m_mobilize]

    # ── Survey / Studies chain ──────────────────────────────────────────────
    survey_chain = _build_chain(
        ids=ids, prefix=DISCIPLINE_PREFIX_BY_CODE['survey'], wbs_code='1.3',
    ...
    m_data_collect = _milestone_activity(ids, MILESTONE_TEMPLATE[3], survey_finish, survey_chain[-1]['id'])
    m_site_survey = _milestone_activity(ids, MILESTONE_TEMPLATE[4], survey_finish, survey_chain[-1]['id'])
    activities += [m_data_collect, m_site_survey]

    # ── HSE study chains (run in parallel, all start after survey) ─────────
    hse_studies = intelligence.get('hse_studies') or []
    hse_finish_dates = []
    for study_name in hse_studies:
        hse_chain = _build_chain(
            ids=ids, prefix=DISCIPLINE_PREFIX_BY_CODE['hse'], wbs_code='1.4',
    ...
    for disc_code in ENGINEERING_DISCIPLINE_ORDER:
        ...
        for index, deliverable in enumerate(deliverables):
            deliverable_chain = _build_chain(
                ids=ids, prefix=prefix, wbs_code='1', discipline=disc_code, role=role,
    ...
    pdr_chain = _build_chain(
        ids=ids, prefix=DISCIPLINE_PREFIX_BY_CODE['pdr'], wbs_code='1',
    ...
    activities.append(_milestone_activity(ids, MILESTONE_TEMPLATE[11], pdr_finish, pdr_chain[-1]['id']))

    epc_chain = _build_chain(
        ids=ids, prefix=DISCIPLINE_PREFIX_BY_CODE['epc'], wbs_code='1',
    ...
    activities.append(_milestone_activity(ids, MILESTONE_TEMPLATE[12], epc_finish, epc_chain[-1]['id']))
    activities.append(_milestone_activity(ids, MILESTONE_TEMPLATE[13], epc_finish, epc_chain[-1]['id']))

    closeout_finish = add_working_days(epc_finish, 5, calendar['working_days_per_week'])
    activities.append(_milestone_activity(ids, MILESTONE_TEMPLATE[14], closeout_finish, epc_chain[-1]['id']))
