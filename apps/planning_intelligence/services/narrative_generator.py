...
optionally augmented by a per-project AI call for prose polish only.
...
schedule data itself — the configured AI provider is only
...
def _augment_executive_summary(default_text: str, facts: dict, project, user) -> str:
    """Return a polished executive summary, or `default_text` unchanged
...
def build_narrative(
    ...
    lines += [
        ...
        'logic' + (
            ', with the Executive Summary optionally polished by the selected AI provider for this project.'
            if project_ai.get_project_ai_config(project) is not None
