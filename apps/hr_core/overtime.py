...
FINAL_POSITIONS = ('hr_manager', 'hr_admin', 'head_hr_administration', 'finance_manager', 'finance_admin')


def is_final_reviewer(user):
    return approval_access(user, 'hr_management') and has_business_position(user, FINAL_POSITIONS)
...
def can_review(task, user):
    ...
    if not approval_access(user, 'hr_management') or not HRWorkflowService.task_is_current(task):
        return False
    if user.pk == instance.employee.user_id:
        return False
    if task.stage.code == 'manager_review':
        return getattr(manager_for_employee(instance.employee), 'user_id', None) == user.pk
    return task.stage.code == 'hr_review' and user.pk != instance.requested_by_id and is_final_reviewer(user)
...
def recipients(task):
    ...
    users = position_users(FINAL_POSITIONS, 'hr_management')
...
def notify_result(overtime):
    ...
    for user in get_user_model().objects.filter(pk__in=ids, is_active=True):
        Notification.objects.create(recipient=user, category=category, title='Overtime request updated',
            ...
            metadata={'overtime_request_id': str(overtime.pk), 'requires_action': False})
