$file = "apps\process_datasheet\mov_threading_processor.py"
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match "STEP 2: Extract HMB") {
        ...
        $newLines += "            logger.warning(f`"[MOV Thread {job_id}] Vision failed/timeout, using mock: {e}`")"
        $newLines += "            log_and_print(f`" [MOV {job_id[:8]}] HMB Vision failed, using mock data...`")"
        ...
        $newLines += "            log_and_print(f`" [MOV {job_id[:8]}] Mock HMB: {len(hmb_data.get('streams', []))} streams`")"
        ...
        Write-Output " Timeout fix applied!"
        break
    }
}
